...
These elements are described in different folders. All the modelling happens within the `openfisca_cov` folder.
...
```sh
git clone https://github.com/openfisca/openfisca-cov.git
cd openfisca-cov
...
Substitute your package's country name for `openfisca_cov` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_cov/situation_examples/couple.json \
